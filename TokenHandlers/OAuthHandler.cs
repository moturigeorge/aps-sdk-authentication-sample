...
namespace aps_sdk_authentication_sample.TokenHandlers
{
    ...
    internal class OAuthHandler
    {
        ...
        /// </summary>
        /// <param name="bearer">The bearer<see cref="ThreeLeggedToken"/>.</param>
        ...
        /// </summary>
        private static TwoLeggedToken InternalToken { get; set; }

        /// <summary>
        /// Get the access token from Autodesk.
        /// </summary>
        /// <param name="scopes">The scopes<see cref="Scopes[]"/>.</param>
        /// <returns>The <see cref="Task{TwoLeggedToken}"/>.</returns>
        static async Task<TwoLeggedToken> Get2LeggedTokenAsync(List<Scopes> scopes)
        {
            var bearer = await authenticationClient.GetTwoLeggedTokenAsync(config.ClientId,
              config.ClientSecret,
              scopes);
            return bearer;
        }
        ...
        /// </summary>
        /// <returns>The <see cref="Task{TwoLeggedToken}"/>.</returns>
        public async Task<TwoLeggedToken> GetInternalAsync()
        {
            if (InternalToken == null || (InternalToken.ExpiresAt.HasValue && DateTime.UtcNow >= DateTimeOffset.FromUnixTimeSeconds(InternalToken.ExpiresAt.Value).UtcDateTime))
            {
        ...
        internal static void _3leggedAsync(AccessTokenDelegate cb)
        {
            ...
            _httpListener.Prefixes.Add(config.Callback.Replace("localhost", "+") + "/");
            try
            {
                _httpListener.Start();
                ...
                string oauthUrl = authenticationClient.Authorize(config.ClientId, ResponseType.Code, redirectUri: config.Callback.ToString(), scopes: _Scopes);
                var file = GetChromeExe();
                string browserPath;
                string args;
                browserPath = file;
                var userData = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "tmp_chrome");
                args = $"/incognito --chrome-frame --user-data-dir={userData} --window-size=540,540 --app={oauthUrl} --disable-application-cache";
        ...
        internal static async void _3leggedAsyncWaitForCode(IAsyncResult ar)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(code))
                {
                    ...
                    var bearer = await authenticationClient.GetThreeLeggedTokenAsync(config.ClientId, code, config.Callback, config.ClientSecret);
